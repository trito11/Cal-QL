...
# CQL + Q-SAM (Sharpness-Aware Minimization for Offline RL)
import os
import random
import uuid
...
from utils import disable_gradients, disable_running_stats, enable_running_stats, soft_update, update_params
...
class TrainConfig:
    ...
    cql_alpha: float = 10.0  # Minimal Q weight (10.0 for halfcheetah, 5.0 for hopper/walker)
    ...
    # Q-SAM / DisCor & WSAM
    rho: float = 1e-4  # SAM perturbation radius
    gamma: float = 0.9  # WSAM weight
    sam_start_step: int = 100000  # Warmup steps before applying SAM on actor
    discor: bool = True  # Whether to use DisCor Bellman error reweighting
    discor_lr: float = 3e-4  # DisCor error network learning rate
    tau_init: float = 10.0  # Initial temperature for DisCor weights
    tau_scale: float = 1.0  # Scaling factor for DisCor temperature
    error_hidden_units: List[int] = field(default_factory=lambda: [256, 256, 256])

    # Wandb logging
    project: str = "CORL"
    group: str = "CQL-QSAM-D4RL"
    name: str = "CQL_Q-SAM"
...
class ReplayBuffer:
    ...
    def load_d4rl_dataset(self, data: Dict[str, np.ndarray]):
        ...
        if n_transitions > self._buffer_size:
            raise ValueError(
                "Replay buffer is smaller than the dataset to load. Increase buffer size."
            )
...
class ReparameterizedTanhGaussian(nn.Module):
    ...
    def log_prob(
        self, mean: torch.Tensor, log_std: torch.Tensor, sampled_action: torch.Tensor
    ) -> torch.Tensor:
        ...
        return torch.sum(action_distribution.log_prob(sampled_action), dim=-1)
    ...
    def forward(
        ...
        if deterministic:
            action_sample = torch.tanh(mean) if not self.no_tanh else mean
        else:
...
class ContinuousCQL_QSAM:
    def __init__(
        ...
        # Q-SAM specific
        discor: bool = True,
        discor_lr: float = 3e-4,
        tau_init: float = 10.0,
        tau_scale: float = 1.0,
        sam_start_step: int = 100000,
        error_hidden_units: List[int] = [256, 256, 256],
    ):
        super().__init__()
        ...
        # DisCor (Error model for critic reweighting)
        self.discor = discor
        self.sam_start_step = sam_start_step
        ...
        if self.discor:
            ...
            self._target_error_net.load_state_dict(self._online_error_net.state_dict())
            disable_gradients(self._target_error_net)

            self._error_optim = Adam(self._online_error_net.parameters(), lr=discor_lr)
            self._tau1 = torch.tensor(tau_init, device=self._device, requires_grad=False)
            self._tau2 = torch.tensor(tau_init, device=self._device, requires_grad=False)
            self._gamma = discount
            self._target_update_coef = soft_target_update_rate
            self.no_tau = tau_init < 1e-6
    ...
    def calc_importance_weights(self, next_states, dones):
        with torch.no_grad():
            ...
            next_errs1, next_errs2 = self._target_error_net(next_states, next_actions)

        dones_col = dones if dones.ndim == 2 else dones.unsqueeze(-1)
        if self.no_tau:
            x1 = -(1.0 - dones_col) * self._gamma * next_errs1
            x2 = -(1.0 - dones_col) * self._gamma * next_errs2
        else:
            x1 = -(1.0 - dones_col) * self._gamma * next_errs1 / (self._tau1 * self.tau_scale)
            x2 = -(1.0 - dones_col) * self._gamma * next_errs2 / (self._tau2 * self.tau_scale)

        batch_size = next_states.shape[0]
        # Self-normalized importance weights scaled by batch_size to maintain loss scale
        imp_ws1 = F.softmax(x1, dim=0) * batch_size
        imp_ws2 = F.softmax(x2, dim=0) * batch_size
        return imp_ws1.squeeze(-1), imp_ws2.squeeze(-1)

    def calc_current_errors(self, states, actions):
        curr_errs1, curr_errs2 = self._online_error_net(states, actions)
        return curr_errs1.squeeze(-1), curr_errs2.squeeze(-1)

    def calc_target_errors(self, next_states, dones, curr_qs1, curr_qs2, target_qs):
        with torch.no_grad():
            next_actions, _ = self.actor(next_states)
            next_errs1, next_errs2 = self._target_error_net(next_states, next_actions)
            next_errs1 = next_errs1.squeeze(-1)
            next_errs2 = next_errs2.squeeze(-1)

            dones_flat = dones.squeeze(-1) if dones.ndim == 2 else dones
            target_errs1 = (curr_qs1 - target_qs).abs() + (1.0 - dones_flat) * self._gamma * next_errs1
            target_errs2 = (curr_qs2 - target_qs).abs() + (1.0 - dones_flat) * self._gamma * next_errs2

        return target_errs1, target_errs2

    def calc_error_loss(self, curr_errs1, curr_errs2, target_errs1, target_errs2):
        err1_loss = torch.mean((curr_errs1 - target_errs1).pow(2))
        err2_loss = torch.mean((curr_errs2 - target_errs2).pow(2))

        soft_update(self._tau1, curr_errs1.detach().mean(), self._target_update_coef)
        soft_update(self._tau2, curr_errs2.detach().mean(), self._target_update_coef)
        soft_update(self._target_error_net, self._online_error_net, self._target_update_coef)
    ...
    def _q_loss(
        ...
        # DisCor reweighting
        if self.discor:
            weights1, weights2 = self.calc_importance_weights(next_observations, dones)
            qf1_loss = torch.mean((q1_predicted - td_target.detach()).pow(2) * weights1)
            qf2_loss = torch.mean((q2_predicted - td_target.detach()).pow(2) * weights2)
        else:
        ...
        return qf_loss, alpha_prime, alpha_prime_loss, td_target, q1_predicted, q2_predicted
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        """ Q function loss """
        qf_loss, alpha_prime, alpha_prime_loss, td_target, q1_predicted, q2_predicted = self._q_loss(
        ...
        if self.total_it <= self.sam_start_step:
            self.actor_optimizer.normalstep()
        else:
            # First SAM step: climb to perturbed point w + e(w)
            self.actor_optimizer.first_step(zero_grad=True)
            disable_running_stats(self.actor)

            # Re-evaluate policy loss at perturbed weights
            new_actions_adv, log_pi_adv = self.actor(observations)
            policy_loss_adv = self._policy_loss(
                observations, actions, new_actions_adv, alpha.detach(), log_pi_adv
            )
            policy_loss_adv.backward()

            # Second SAM step: update parameters with sharpness-regularized gradient
            self.actor_optimizer.second_step(zero_grad=True)
        ...
        # DisCor error model update
        if self.discor:
            curr_errs1, curr_errs2 = self.calc_current_errors(observations, actions)
            target_errs1, target_errs2 = self.calc_target_errors(
                next_observations, dones, q1_predicted.detach(), q2_predicted.detach(), td_target.detach()
            )
            err_loss = self.calc_error_loss(curr_errs1, curr_errs2, target_errs1, target_errs2)
            update_params(self._error_optim, err_loss)
            log_dict["error_loss"] = err_loss.item()
            log_dict["tau1"] = self._tau1.item()
            log_dict["tau2"] = self._tau2.item()
    ...
    def state_dict(self) -> Dict[str, Any]:
        ...
        if self.discor:
            state["online_error_net"] = self._online_error_net.state_dict()
            state["target_error_net"] = self._target_error_net.state_dict()
            state["error_optim"] = self._error_optim.state_dict()
        return state
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]):
        ...
        if self.use_automatic_entropy_tuning and "sac_log_alpha_optim" in state_dict:
        ...
        if self.discor and "online_error_net" in state_dict:
            self._online_error_net.load_state_dict(state_dict["online_error_net"])
            self._target_error_net.load_state_dict(state_dict["target_error_net"])
            self._error_optim.load_state_dict(state_dict["error_optim"])
...
def train(config: TrainConfig):
    ...
    # Actor WSAM optimizer
    base_actor_optimizer = torch.optim.Adam(actor.parameters(), config.policy_lr)
    actor_optimizer = WeightedSAM(
        actor,
    ...
    kwargs = {
        ...
        # Q-SAM specific
        "discor": config.discor,
        "discor_lr": config.discor_lr,
        "tau_init": config.tau_init,
        "tau_scale": config.tau_scale,
        "sam_start_step": config.sam_start_step,
        "error_hidden_units": config.error_hidden_units,
    }

    print("---------------------------------------")
    print(f"Training CQL + Q-SAM, Env: {config.env}, Seed: {seed}")
    print("---------------------------------------")

    trainer = ContinuousCQL_QSAM(**kwargs)
